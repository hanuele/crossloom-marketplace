...
- **`.mcp.json`** — the MCP server *registration* (`${CLAUDE_PLUGIN_ROOT}/bin/cl-mcp` since plugin 0.5.1 — one extensionless name, two files: `cl-mcp.cmd` on Windows, `#!/bin/sh` `cl-mcp` elsewhere, each picking the first interpreter that can import the wheel; the single polyglot `bin/cl-mcp.cmd` in 0.5.0; `python -m crossloom_cli.mcp.server` in 0.4.1; `cl mcp serve` before). This is **wiring, not
